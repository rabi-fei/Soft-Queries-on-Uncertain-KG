...
@torch.no_grad()
def solve_soft_EFO1(DNF_formula: DisjunctiveFormula, relation_matrix, conjunctive_tnorm, existential_tnorm, index, device,
               max_enumeration):
    ...
    with torch.no_grad():
        ...
        for sub_formula in DNF_formula.formula_list:
            ...
            sub_ans = solve_soft_conjunctive(sub_kg, neg_kg, relation_matrix,
...
def existential_update(leaf_node, adjacency_node, sub_graph: KnowledgeGraph, neg_sub_graph: KnowledgeGraph,
    ...
    all_prob_matrix = construct_matrix_list(leaf_node, adjacency_node, sub_graph, neg_sub_graph, r_matrix_list, conj_tnorm)
    if conj_tnorm == 'Plus':
...
def constant_update(constant_node, adjacency_node, sub_graph: KnowledgeGraph, neg_sub_graph: KnowledgeGraph,
                       r_matrix_list, cons_andidates, adj_candidates, conj_tnorm, exist_tnorm) -> dict:
...
def cut_node_sub_problem_soft(to_cut_node, adjacency_node_list, sub_graph: KnowledgeGraph, neg_sub_graph: KnowledgeGraph,
    ...
    for adjacency_node in adjacency_node_list:
        if "s" in to_cut_node:
            adj_candidate_vec = constant_update(to_cut_node, adjacency_node, sub_graph, neg_sub_graph, r_matrix_list,
    ...
    sub_answer = solve_soft_conjunctive(new_sub_graph, new_sub_neg_graph, r_matrix_list, new_candidate_set, conj_tnorm,
...
@torch.no_grad()
def solve_soft_conjunctive(positive_graph: KnowledgeGraph, negative_graph: KnowledgeGraph, relation_matrix,
    ...
    if now_leaf_node:  # If there exists leaf node in the query graph, always possible to shrink into a sub_problem.
        ...
        if being_asked_variable:
            ...
            sub_ans = solve_soft_conjunctive(sub_pos_g, sub_neg_g, relation_matrix, now_candidate_set,
