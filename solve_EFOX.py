...
@torch.no_grad()
def solve_conjunctive(pos_qg: KnowledgeGraph, neg_qg: KnowledgeGraph, relation_matrix,
                      now_candidate_set: dict, conjunctive_tnorm, existential_tnorm, free_variable_list, device,
                      max_enumeration):
    n_entity = relation_matrix[0].shape[0]
    if not pos_qg.triples and not neg_qg.triples:
        return get_final_answer(pos_qg, relation_matrix, conjunctive_tnorm, existential_tnorm, device)
    if len(now_candidate_set) == 1:
        return now_candidate_set
    now_leaf_node, adjacency_node, _ = \
        find_leaf_node(pos_qg, neg_qg, now_candidate_set, free_variable_list)
    if now_leaf_node:  # If there exists leaf node in the query graph, always possible to shrink into a sub_problem.
        ...
        answer, cut_node_type = cut_node_sub_problem(
            now_leaf_node, adjacency_node_list, pos_qg, neg_qg, relation_matrix, now_candidate_set, conjunctive_tnorm,
            existential_tnorm, free_variable_list, device, max_enumeration)
        extend_ans()
    else:
        to_enumerate_node, adjacency_node_list = find_enumerate_node(pos_qg, neg_qg, now_candidate_set,
                                                                     free_variable_list)
        ...
        for i, enumerate_candidate in enumerate(to_enumerate_candidates):
            ...
            answer, cut_type = cut_node_sub_problem(to_enumerate_node, adjacency_node_list, pos_qg, neg_qg, relation_matrix,
                                          now_candidate_set, conjunctive_tnorm, existential_tnorm, free_variable_list,
                                          device, max_enumeration)
...
def find_enumerate_node(query_graph: QueryGraph, now_candidate, now_variable):
    return_candidate = [None, 100, 100000]
    for node in now_candidate:
        if node == now_variable:
            continue
        adjacency_node_list = list(set.union(*[query_graph.h2t[node], query_graph.t2h[node]]))
...
def cut_node_sub_problem(to_cut_node, adjacency_node_list, sub_graph: KnowledgeGraph, neg_sub_graph: KnowledgeGraph,
                         r_matrix_list, now_candidate_set, conj_tnorm, exist_tnorm, free_variable_list, device,
                         max_enumeration):
    ...
    if to_cut_node in free_variable_list:
        ...
        if len(adjacency_node_list) == 1 and adjacency_node_list[0] not in free_variable_list:
            new_free_variable_list.append(adjacency_node_list[0])
    ...
    sub_answer = solve_conjunctive(new_sub_graph, new_sub_neg_graph, r_matrix_list, new_candidate_set, conj_tnorm,
                                   exist_tnorm, free_variable_list, device, max_enumeration)
    return sub_answer, cut_node_type
...
@torch.no_grad()
def extend_ans(to_extend_node, previous_node, sub_graph: KnowledgeGraph, neg_sub_graph: KnowledgeGraph, relation_matrix,
               leaf_candidate, sub_ans, conj_tnorm, exist_tnorm, sub_free_variable_list, cut_node_type):
    all_prob_matrix = construct_matrix_list(previous_node, to_extend_node, sub_graph, neg_sub_graph, relation_matrix,
                                            conj_tnorm)
    if cut_node_type == 'cut_existential_or_constant':
        pass
    elif cut_node_type == 'cut_free_e':
        if len(sub_free_variable_list) == 1:  # In this case, the sub_ans is now a vector
            if conj_tnorm == 'product':
            ...
            if conj_tnorm == 'product':
                ...
                raise NotImplementedError
            return prob_vec
        elif len(sub_free_variable_list) == 2:  # In this case, the sub_ans is already n*n matrix
            if conj_tnorm == 'product':
                all_prob_matrix.mul_(sub_ans.unsqueeze(-1))
            elif conj_tnorm == 'Godel':
                all_prob_matrix = torch.minimum(all_prob_matrix, sub_ans.unsqueeze(-1))
...
@torch.no_grad()
def get_final_answer(query_graph: QueryGraph, relation_matrix,
                      now_candidate_set: dict, conjunctive_tnorm, existential_tnorm, now_variable_list, device):
    torch.count_nonzero(now_candidate_set[to_enumerate_node])


@torch.no_grad()
def solve_EFOX(DNF_formula:DisjunctiveFormula, relation_matrix, conjunctive_tnorm, existential_tnorm, index, device,
               max_enumeration):
    ...
    with torch.no_grad():
        ...
        for sub_formula in DNF_formula.formula_list:
            ...
            sub_query_graph = QueryGraph(sub_formula)
            edge_attribute = sub_query_graph.edge_grounded_rel_id * sub_query_graph.pos_edge
            sub_query_matrix = to_dense_adj(sub_query_graph.edge_index, edge_attribute)
            sub_ans = solve_conjunctive(sub_query_matrix, relation_matrix,
...
if __name__ == "__main__":
    ...
    for ifof, fof in t:
        ...
        for query_index in range(len(fof.easy_answer_list)):
            ans = solve_EFOX(fof, relation_matrix_list, args.c_norm, args.e_norm, query_index, cuda_device, args.max)
