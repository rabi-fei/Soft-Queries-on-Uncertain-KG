...
parser.add_argument("--num_negative", type=int, default=200)
...
parser.add_argument("--start_index", type=int, default=9)
...
def sample_one_formula_query(given_lstr, part_kg: KnowledgeGraph, full_kg: KnowledgeGraph, num_samples, sample_mode,
    ...
    all_query_list = []
    now_index = -1
    full_matrix = full_matrix if full_matrix is not None else None
            ...
            while qa_dict is None:
                if meaningful_negation and negation_tolerance:
                    if now_negation_tolerance >= negation_tolerance:
                        qa_dict, full_answer, epfo_answers = fof.sample_query(full_kg, False, full_matrix, sample_max_ans)
                        now_negation_tolerance = 0
                    else:
                        qa_dict, full_answer, epfo_answers = fof.sample_query(full_kg, True, full_matrix, sample_max_ans)
            ...
            if qa_dict and str(qa_dict) not in stored_qa_dict:  # We notice sampling may fail and return None
                ...
                if full_answer and full_answer != part_answer:
                    ...
                    if sample_mode == 'train':
                        ...
                        integer_list = [int(item) for item in list(full_answer.keys())]#TODO:test!
