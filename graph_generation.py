...
class FamilySimulator:
    ...
    def dump_tsv(self, loss_ratio, target_file):
        lines = []
...
def family_kg_generate(
    ...
    simulator.dump_tsv(
        loss_ratio=-1, target_file=os.path.join(target_path, "family-full.tsv"))

    for lr in loss_ratios:
        simulator.dump_tsv(
            loss_ratio=lr, target_file=os.path.join(target_path, f"family-loss-{lr}.tsv"))
