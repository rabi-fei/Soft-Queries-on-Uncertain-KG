...
from Neural_SIU import SIU_finetune
...
from src.structure.geometric_graph import SoftQueryGraph
...
#['r1(s1,f1,0%,1.0)', '(r1(s1,e1,0%,1.0))&(r2(e1,f1,0%,1.0))']
...
#    torch.autograd.set_detect_anomaly(True)
    ...
    for formula in query_data:
        ...
        efo1_ans_list = [ans_dict[f"{f_str}_answers"] for ans_dict in query_data[formula].easy_answer_list]
        efo1_val_list = [ans_dict[f"{f_str}_values"] for ans_dict in query_data[formula].hard_answer_list]
        answer_list.extend(efo1_ans_list)
        value_list.extend(efo1_val_list)
    pred_embedding = torch.cat(emb_list, dim=0)
    all_positive_logit, all_negative_logit, all_subsampling_weight = model.criterion(pred_embedding, answer_list, value_list)
...
def eval_batch_query(model, pred_emb_list, easy_ans_list, hard_ans_list):
    ...
    with torch.no_grad():
            all_logit = model.compute_all_entity_logit(pred_emb_list, union = len(pred_emb_list)>1)
            # batch*nentity
            ...
            for i in range(all_logit.shape[0]):
                ans = easy_ans_list[i]["f1_answers"][::-1]
                value = hard_ans_list[i]["f1_values"][::-1]
                num_ans = len(ans)
                ...
                if  num_ans > 1:
                    ...
                    metrics['num_correlation_queries'] += 1

            num_query = all_logit.shape[0]
            metrics['num_queries'] += num_query
...
def eval_step(data_path, model, configure, device, step, writer=None):
    with torch.no_grad():
        ...
        for ifof, fof in t:
            if model.name != 'FIT':
                QG_embedding_list = []
                for fof_formula in fof.formula_list:
                    QG_instance = SoftQueryGraph(fof_formula, device)
                    QG_embedding_list.extend(QG_instance.get_whole_graph_embedding(model=model))
            else:
        ...
        writer.save_json(all_metrics, f"all_logging_test_{step}_{formula_id}.pickle")
        return all_metrics
...
if __name__ == "__main__":
    ...
    if model_name == 'beta':
        ...
        model = SIU_finetune(**model_params)
    else:
    ...
    if 'train' not in configure['action']:
        assert train_config['steps'] != init_step  #TODO
                ...
                if model_name == 'SIU':
                    ...
                    if train_other_tm:
                        ...
                        if model_name != 'FuzzQE':
                            _log_second = train_step(model, opt, train_primary_tm, loss_function)
                ...
                if 'valid' in configure['action']:
                    ...
                    for i, row in tqdm.tqdm(all_formula_data.iterrows(), total=len(all_formula_data)):
                        ...
                        if not osp.exists(data_path):
                            ...
                            continue
                        all_metric_formula = eval_step(data_path, model, configure, device, step, writer)
                        for metric in all_metric_formula:
                            if metric != 'num_queries':
                                all_metric_formula[metric] /= all_metric_formula['num_queries']
                        all_metrics[formula] = all_metric_formula
                    writer.save_json(all_metrics, f"all_logging_valid_{step}.json")
