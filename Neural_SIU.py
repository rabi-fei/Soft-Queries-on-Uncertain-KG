...
class Kge_finetune(nn.Module):
    ...
    def create_whole_matrix(self, rel_id):
        ...
        for head_batch_tensor in batch_head_list:
            ...
            #for batch_index in range(batch_score.shape[0]):
            ...
            sparse_batch_prob = torch.where(
                        batch_score > self.threshold, batch_score,
                        torch.zeros_like(batch_score).to(self.device))
            clamp_batch_prob = torch.clamp(sparse_batch_prob, 0, 1-self.epsilon)
                #clamp_batch_prob[list(observed_tail_set)] = 1
            rel_matrix[head_batch_tensor[0][0].item() : head_batch_tensor[-1][0].item() + 1] = clamp_batch_prob
            del batch_score
        #sparse_matrix = rel_matrix.to('cpu').to_sparse()
        return rel_matrix


class SIU_finetune(nn.Module):
    """
    ...
    def __init__(self, n_entity, n_relation, freeze_ent, negative_sample_size, train_kg: KnowledgeGraph, ukge_path: str,
        ...
        elif self.kge == 'UKGE_numpy':
            with open(ukge_path, "rb") as handle:
                params_dict = pickle.load(handle)
            ent_emb = torch.tensor(params_dict['entity_embedding']).to(device)
            rel_emb = torch.tensor(params_dict['relation_embedding']).to(device)
            w, b = params_dict["w"], params_dict["b"]
            self.score_function = compute_batch_score_ukge
        elif self.kge == "UKGE":
            model_param = torch.load(ukge_path, map_location=self.device)
    ...
    def solve_fof(self, fof, mode):
        ...
        if mode == "train":
            ...
            assert len(self.stored_matrix_list) > 0
            for query_index in range(len(fof.pred_grounded_relation_id_dict['r1'])):
    ...
    def criterion(self, pred_ans, answer_list, answer_values):
        ans_vec_list = []
        base_num = 4
    ...
    def construct_all_matrices(self):
        with torch.no_grad():
            ...
            for rel_id in range(self.n_relation):
                ...
                print("sparse ratio:", (rel_matrix > 0).sum().item() / (rel_matrix.shape[0] * rel_matrix.shape[1]))
...
def neural_solve_EFO1_X(DNF_formula, kgematrix: Kge_finetune, conjunctive_tnorm, existential_tnorm, index, device,
    ...
    for sub_formula in DNF_formula.formula_list:
        ...
        for pred in sub_formula.predicate_dict.values():
            pred_triples = (pred.head.name, sub_formula.pred_grounded_relation_id_dict[pred.name][index],
                            pred.tail.name, pred.alpha_float_list[index], pred.beta_float_list[index])
            if pred.negated:
    ...
    if len(sub_ans_list) == 1:
        if DNF_formula.term_grounded_alpha_beta_dict["a1"][index] > 0:
            pred_vec = torch.zeros_like(sub_ans_list[0], device=sub_ans_list[0].device)            
            pred_vec.scatter_(0, torch.nonzero(sub_ans_list[0]).squeeze(), 
                              torch.log(sub_ans_list[0][torch.nonzero(sub_ans_list[0]).squeeze()]))
        else:
            pred_vec = sub_ans_list[0]
...
def construct_matrix_dynamic(head_node, tail_node, sub_graph, neg_sub_graph, kge_matrix: Kge_finetune, conj_tnorm,
    ...
    for r in h2t_relation:
        alpha, beta = [(rab[1], rab[2]) for rab in sub_graph.ht2rab[(head_node, tail_node)] if rab[0] ==r][0]
        #if alpha < 0.01:
        #    print("here")
        alpha_list.append(alpha)
        beta_list.append(beta)
        transit_matrix_list.append(kge_matrix.forward(r, head_ent_vec,
                                                      train_mask=False if mode == 'train' else False)) #TODO!
    for r in t2h_relation:
        alpha, beta = [(rab[1], rab[2]) for rab in sub_graph.ht2rab[(tail_node, head_node)] if rab[0] ==r][0]
        alpha_list.append(alpha)
        beta_list.append(beta)
    ...
    for r in h2t_negation:
        alpha, beta = [(rab[1], rab[2]) for rab in neg_sub_graph.ht2rab[(head_node, tail_node)] if rab[0] ==r][0]
        #if alpha < 0.01:
        #    print("here")
        alpha_list.append(alpha)
        beta_list.append(beta)
    ...
    for r in t2h_negation:
        alpha, beta = [(rab[1], rab[2]) for rab in neg_sub_graph.ht2rab[(tail_node, head_node)] if rab[0] ==r][0]
        alpha_list.append(alpha)
        beta_list.append(beta)
        transit_matrix_list.append(1 - kge_matrix.forward(r, head_ent_vec,
