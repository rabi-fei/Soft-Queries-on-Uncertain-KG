...
class UKGE(nn.Module, NeuralBinaryPredicate):
    ...
    def embedding_score(self, head_emb, rel_emb, tail_emb):
        ...
        """
        return  self.w * (head_emb * rel_emb * tail_emb).sum(1) + self.b
    ...
    #     ranking_score = self.w * (batch_embedding_input * self.entity_embedding).sum(1) + self.b
    ...
    def load_params(self, params_path):
        ...
        self._entity_embedding.weight.data.copy_ = torch.from_numpy(params_dict["entity_embedding"]).float()
        self._relation_embedding.weight.data.copy_ = torch.from_numpy(params_dict["relation_embedding"]).float()
        self.w = params_dict["w"]
        self.b = params_dict["b"]
        self._entity_embedding.weight.data
        # self._entity_embedding.weight.data.required_grad = False
        # self._relation_embedding.weight.data.required_grad = False
